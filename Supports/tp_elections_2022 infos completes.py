...
# Structure : Code, Libellé, Inscrits, Abstentions, Votants, Blancs, Nuls, Exprimés
# Puis pour chaque candidat : Sexe, Nom, Prénom, Voix (répété 12 fois)
types_colonnes = {
    ...
    'Nuls': int,
    'Exprimés': int,
    # Les colonnes Sexe, Nom, Prénom sont des str
    # Les colonnes Voix sont des int
}
...
# Identifier les colonnes "Voix" (toutes les 4 colonnes après Exprimés, on prend la 4ème)
colonnes_voix = [col for col in contenu.columns if col == 'Voix']

# Méthode alternative : récupérer les colonnes par index
# Les voix commencent à la colonne 11 (index 11) et se répètent tous les 4 colonnes
colonnes_voix_indices = list(range(11, len(contenu.columns), 4))
noms_candidats_indices = list(range(9, len(contenu.columns), 4))  # Colonnes "Nom"
prenoms_candidats_indices = list(range(10, len(contenu.columns), 4))  # Colonnes "Prénom"

# Créer les labels des candidats (Prénom NOM)
labels_candidats = []
for i in range(len(noms_candidats_indices)):
    nom_col = contenu.columns[noms_candidats_indices[i]]
    prenom_col = contenu.columns[prenoms_candidats_indices[i]]
    # Prendre le premier département pour avoir les noms
    nom = contenu.iloc[0][nom_col]
    prenom = contenu.iloc[0][prenom_col]
    labels_candidats.append(f"{prenom} {nom}")

if colonnes_voix_indices:
    for idx, row in contenu.iterrows():
        dept = row['Libellé du département']
        
        # Récupérer les voix de chaque candidat
        voix_candidats = [row[contenu.columns[i]] for i in colonnes_voix_indices]
        
        plt.figure(figsize=(10, 10))
        plt.pie(voix_candidats, labels=labels_candidats, autopct='%1.1f%%', startangle=90)
        plt.title(f"Répartition des voix par candidat - {dept}")
        plt.tight_layout()
        plt.savefig(f"images/candidats_{dept.replace(' ', '_').replace('/', '_')}.png")
        plt.close()
    
    # Diagramme pour l'ensemble de la France
    total_voix = [contenu.iloc[:, i].sum() for i in colonnes_voix_indices]
    
    plt.figure(figsize=(12, 12))
    plt.pie(total_voix, labels=labels_candidats, autopct='%1.1f%%', startangle=90)
